...
def test_init_type(gym, np, torch, functools, REINFORCEWithBaselinePolicy, policy_init_network, value_init_network):
    torch.manual_seed(0)
    np.random.seed(0)
    env = gym.make('CartPole-v0')
    test_state, _ = env.reset(seed = 0)
    assert_equal = functools.partial(torch.testing.assert_close, rtol=0, atol=0)
    test_policy = REINFORCEWithBaselinePolicy(env, policy_init_network(env), value_init_network(env), 0.99)
...
def test_update_type(gym, np, torch, functools, REINFORCEWithBaselinePolicy, policy_init_network, value_init_network, generate_episode):
    torch.manual_seed(0)
    np.random.seed(0)
    test_env = gym.make('CartPole-v0')
    assert_equal = functools.partial(torch.testing.assert_close, rtol=0, atol=0)
    test_reinforce = REINFORCEWithBaselinePolicy(test_env, policy_init_network(test_env), value_init_network(test_env), 0.99)
...
@test_case(points=2, hidden=False)
def test_update_type_1(gym, np, torch, functools, REINFORCEWithBaselinePolicy, policy_init_network, value_init_network):
